...
export const refreshUserGiftsAPI = async (): Promise<ApiServiceResponse<{ status: string, task_id?: string, message: string }>> => {
  const response = await ApiService.post<{ status: string, task_id?: string, message: string }>({
